...
install() {
  ...
  [[ -d ${SCHEMES_DIR}/Tellgo.colors ]] && rm -r ${SCHEMES_DIR}/Tellgo.colors
  ...
  [[ ${color} == '' ]] && \
  cp -ur ${SRC_DIR}/color-schemes/Tellgo.colors                                    ${SCHEMES_DIR} && \
  cp -ur ${SRC_DIR}/color-schemes/Tellgo.colors                                    ${PLASMA_DIR}/${name}/colors

  [[ ${color} == '-light' ]] && \
  cp -ur ${SRC_DIR}/color-schemes/Tellgo.colors                               ${SCHEMES_DIR}
}

echo "Installing 'Tellgo kde themes'..."
